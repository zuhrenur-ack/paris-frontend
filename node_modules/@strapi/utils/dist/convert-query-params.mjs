   ...
   */ const convertLimitQueryParams = (limitQuery)=>{
    ...
    const convertPopulateObject = (populate, schema)=>{
        ...
        return Object.entries(populate).reduce((acc, [key, subPopulate])=>{
            ...
            if (isMorphLikeRelationalAttribute) {
                ...
                if (hasInvalidProperties) {
                    throw new ValidationError(`Invalid nested populate for ${schema.info?.singularName}.${key} (${schema.uid}). Expected a fragment ("on") or "count" but found ${JSON.stringify(subPopulate)}`);
                }
                /**
         ...
         */ if ('populate' in subPopulate && subPopulate.populate !== '*') {
                    throw new ValidationError(`Invalid nested population query detected. When using 'populate' within polymorphic structures, ` + `its value must be '*' to indicate all second level links. Specific field targeting is not supported here. ` + `Consider using the fragment API for more granular population control.`);
                }
